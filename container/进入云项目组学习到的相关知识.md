...
因此我们应该很容易理解，Infra容器会占极少的资源，因此它使用的是一个用汇编语言所写的镜像`k8s.gcr.io/pause`运行生成，其解压之后也仅有100KB左右。我们可以通过这样的yaml文件运行出来一个管理nginx和busybox的Pod看看它里面是什么样的：
...
通过上述命令，我们就成功创建了一个pod，暂时不用管yaml文件的含义，我们可以从执行结果看到infra容器的主进程成为了此Pod的PID==1的超级进程，说明了Pod是组合而成的：
